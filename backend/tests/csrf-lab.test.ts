...
test("builds a Lax CSRF lab cookie", () => {
  const header = createCsrfLabCookie("Lax");
  assert.match(header, new RegExp(`^${CSRF_LAB_COOKIE_NAME}=csrf-[a-f0-9]{32}; Path=/; SameSite=Lax$`));
});

test("builds a Strict CSRF lab cookie", () => {
  const header = createCsrfLabCookie("Strict");
  assert.match(header, new RegExp(`^${CSRF_LAB_COOKIE_NAME}=csrf-[a-f0-9]{32}; Path=/; SameSite=Strict$`));
});
...
test("builds a clearing header", () => {
  assert.equal(
    buildClearedCsrfLabCookie(),
    `${CSRF_LAB_COOKIE_NAME}=; Path=/; SameSite=Lax; Max-Age=0`,
  );
});
